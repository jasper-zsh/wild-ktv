...
from .. import Lyrics, LyricsData, LyricsLine, BaseLyricsProvider
from ..provider.ne import NeteaseMusicLyricsProvider
from ..provider.qm import QQMusicLyricsProvider
from ..enum import LyricsFormat, LyricsType
...
def convert2(lyrics: Lyrics,
    ...
    if lyrics_format == LyricsFormat.JSON:
        ...
        for key, value in vars(lyrics).items():
            if key in ("source", "title", "artist", "album", "id", "mid", "duration", "accesskey"):
                if key == "source":
                    value: BaseLyricsProvider
                    json_dict["info"][key] = value.__class__.__name__
    ...
    langs_order: list[str] = [lang for lang in cfg_langs_order if lang in langs and lang in lyrics_dict]
...
def find_closest_match(data1: LyricsData, data2: LyricsData, data3: LyricsData | None = None, source: BaseLyricsProvider | None = None) -> dict[int, int]:
    """为原文匹配其他语言类型的歌词
    ...
    :return: 匹配结果(引索)
    """
    if isinstance(source, NeteaseMusicLyricsProvider) and data3:
        data3_matched = find_closest_match(data3, data2, source=source)
        matched = {}
    ...
    if isinstance(source, QQMusicLyricsProvider):
