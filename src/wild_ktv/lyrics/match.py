...
async def match_lyrics(provider: BaseLyricsProvider, song: Song, min_score: float = 60):
    artist = '/'.join(map(lambda x: x.name, await song.awaitable_attrs.artists))
    keyword = f"{artist} - {song.name}" if len(artist) > 0 else song.name
    result = await _search(provider, song, keyword, min_score)
    if not result:
    ...
    await provider.get_lyrics(lyrics)
    if lyrics:
        logger.info(f'成功获取歌词：{keyword}')
        return lyrics
    
async def _search(provider: BaseLyricsProvider, song: Song, keyword: str, min_score: float = 60):
    artist = '/'.join(map(lambda x: x.name, song.artists))
    search_result = await provider.search(keyword, SearchType.SONG)

    duration = None # 暂时不进行时长匹配
    score_info: list[tuple[float, dict]] = []
    for info in search_result:
        if duration and abs(info.get('duration', -100) - duration) > 3:
            continue
        # if not duration:
        #     logger.warning("没有获取到 %s - %s 的时长, 跳过时长匹配检查", artist, song.name)

        artist_score = None
        title_score = calculate_title_score(song.name, info.get('title', ''))
        album_score = max(text_difference('', info.get('album', '').lower()) * 100, 0)  # 没有专辑信息
        ...
        elif '' and info.get('album', '').strip():  # 没有专辑信息
